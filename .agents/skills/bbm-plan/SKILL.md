...
| `--accumulated_num` | integer (tokens per request) | *unset* — **every plan run derives a default**: `1200` with the stock prompts (up to `1600` under a fat custom `--prompt`), a flat `800` on an endpoint without a strict-schema verdict — except session runs (`codex` included), which keep the un-halved value; the run narrates the number and route class it chose | a book where you want a different cost/latency point than the derived one: consecutive units of any length share one request up to N tokens, and a typed value always wins un-halved. The default is an owner-set safety margin at a quarter of the largest budget measured fault-free (`4800`, at the old 32-unit cap, weak models included); raising it buys per-content cost back but spends that margin — only 1600–4800 was measured clean, and only on the evaluated models, so treat values past the default as the operator's own risk. An explicit `1` is the off switch in any mode. Interrupted runs checkpoint and `--resume` either way |
