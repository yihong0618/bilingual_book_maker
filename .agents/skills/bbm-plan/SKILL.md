...
| `--batch_units` | integer (units per request) | `16` — the ceiling a gpt-5.6-luna eval found fault-free | a weaker or reseller model: every content fault the eval saw was in large-unit-count batches, so drop to `8` (or `4`) there. An endpoint that verifies JSON mode but not a strict schema carries half the cap automatically. Never raise past `16` — faults lived at 50 |
