...
| `--context-compact-at` | estimated-token budget, minimum 500 | **unset → 8192**, pinned, printed at start — every session run, grouped or not, codex included | leave it unset: the fewest-seams edge of the measured band (its cheapest cell), kept high so the smaller per-request budgets don't multiply compactions. Setting it lower (toward 2000–4000) squeezes the last ~10–25% of session cost at the price of more seams; past 16000 the cost wall is steep and the only drift ever observed lived in the long-window cell. Set it only when the user insists on a number — an explicit value always wins. **Needs `--use_context session`** on an API route; without it the flag is accepted and does nothing. On `codex` it always applies |
