...
|---|---|---|---|
| openai (any OpenAI-shaped entry, `orcarouter`, and `groq`/`xai`/`litellm`, which are that route at their own address) | `(--provider NAME)` | `(--use_context session)` | one cached history, compacted at 4096 by default (`--context-compact-at` overrides); costs less than window mode for several times the context |
...
| `--context-compact-at` | estimated-token budget, minimum 500 | **unset → 4096**, pinned, printed at start — every session run, grouped or not, codex included | leave it unset: 4096 sits in the measured flat-cost region (cheapest cell on 2 of 3 books; solved per-book optima 1580–2512) and away from the long-window drift cell — an owner-set margin that buys extra seams knowingly. Set it only when the user insists on a number — an explicit value always wins. **Needs `--use_context session`** on an API route; without it the flag is accepted and does nothing. On `codex` it always applies |
...
| `--accumulated_num` | integer (tokens per request) | *unset* — **every plan run derives a default**: `1200` with the stock prompts (up to `1600` under a fat custom `--prompt`), a flat `800` on an endpoint without a strict-schema verdict — except session runs (`codex` included), which keep the un-halved value; the run narrates the number and route class it chose | a book where you want a different cost/latency point than the derived one: consecutive units of any length share one request up to N tokens, and a typed value always wins un-halved. The default is an owner-set safety margin at a quarter of the largest budget measured fault-free (`4800`, at the old 32-unit cap, weak models included); per-content-token cost only falls with size, so raising it is a cost knob, not a risk knob; an explicit `1` is the off switch in any mode. Interrupted runs checkpoint and `--resume` either way |
| `--max-batch-units` | integer (units per request) | `16` — a quarter of the measured fault-emergence level (a 923-request sweep put the first content faults at 64 effective units, prose; everything through 48 read back clean): an owner-set margin, double the old half-way distance | the run keeps printing misalignment recoveries: drop to `8` (or `4`) — the retries, not faults, are what a lower cap buys back. An endpoint that verifies JSON mode but not a strict schema carries half the cap automatically (effective `8`), and that half is where reply miscounts actually live, so don't undo it by doubling. Never raise past `48` — faults emerged at 64, and content per request is bounded by `--accumulated_num` either way |
