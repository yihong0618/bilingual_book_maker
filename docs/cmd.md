...
| `--no_disclosure` | Do not mark the epub as an AI translation, or a machine translation on the engine formats (translator credit, description line, closing note). Silences `--translation-metadata`'s machine record too. |
| `--translation-metadata` | Record how the file was made, invisibly: the full record is `bbm_translation_metadata.json` in the book (build, model, endpoint host, sanitized command line, languages, date, glossary sha256 — conversions keep manifest files, so it survives); the package metadata beside it is just three `bbm:` entries (tool marker with the build, model, date), since metadata is trivially stripped. Never the key or the `--prompt` text. Automatic on plan-mode and session runs; this is the tag-mode opt-in. A `--glossary` file is embedded verbatim; learned terms never are. |
...
| `--accumulated_num N` | EPUB token/character accumulation and SRT subtitle-block character batching (capped at 512 for SRT). In EPUB plan mode it is a per-request token budget: consecutive units of any length share one request up to `N` tokens (at most `--max-batch-units` units per request; half that when the endpoint verifies JSON mode but not a strict schema). Untyped, every plan run derives a default from the run's own prompt overhead — `1600` with the stock prompts, up to `2000` under a fat custom `--prompt` — halved per request (floor `800`) on an endpoint without a strict-schema verdict, the same margin that halves the unit cap there; session runs (`codex` included) keep the un-halved value, their measured default. The run narrates the number and the route class. Pass `1` to turn grouping off. Minimum `1`. |
