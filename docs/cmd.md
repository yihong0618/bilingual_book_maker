...
| `--no_disclosure` | Do not mark the epub as an AI translation, or a machine translation on the engine formats (translator credit, description line, closing note). Turns off `--translation-metadata` too. |
| `--translation-metadata` | Write translation metadata into the book: `bbm_translation_metadata.json` (build, model, endpoint host, command line, languages, date, glossary sha256 — format conversions keep manifest files, so it survives them) plus three `bbm:` package metas (tool build, model, date). API keys and the `--prompt` text are never written. Plan-mode and session runs write it by themselves; this flag adds it to a plain tag-mode run. A `--glossary` file is embedded alongside; terms a run learned by itself are not. |
